...
# SL.append(shape=shape_dict.plank(l=l, s=s), count=1, meshsize=meshsize, material=material_dict.peridem_deformable(delta, G_scale=1, K_scale=1, Gnot_scale=1, rho_scale=1))
# debug
SL.append(shape=shape_dict.plank(l=l, s=s), count=1, meshsize=meshsize, material=material_dict.sodalime_similar_to_peridem(delta))

# save Young's modulus to file
# print('Youngs modulus', SL.material_list[0].E)
with open('output/csv/bar_youngs.csv', 'w') as f:
    f.write(str(SL.material_list[0].E))
