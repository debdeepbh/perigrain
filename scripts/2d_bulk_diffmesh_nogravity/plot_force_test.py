...
for i in range(t_i):
    ...
    # reverse axis
    # plt.plot([ np.abs(v.wall_reaction[2,1]) for v in d[i]],  phi, label = r'$\sigma=$ '+labels[i])

    # volume of the gap, negative gap
    # gap = [vols[i] - (v.wall_v * v.wall_h) for v in d[i]]
    # plt.plot(gap, [ np.abs(v.wall_reaction[2,1]) for v in d[i]],  label = r'$\sigma=$ '+labels[i])

    # strain
    # ov = d[i][0].wall_v
    # strain = [ (ov -  v.wall_v)/ov   for v in d[i]]
    # plt.plot(strain, [ np.abs(v.wall_reaction[2,1]) for v in d[i]],  label = r'$\sigma=$ '+labels[i])
