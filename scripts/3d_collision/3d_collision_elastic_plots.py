...
for t in range(plti.fc, plti.lc+1):
    ...
    ua.append(np.mean(A.CurrPos[:,2]))
    ub.append(np.mean(B.CurrPos[:,2]))

    va.append(np.mean(A.vel[:,2]))
    vb.append(np.mean(B.vel[:,2]))
...
save_filename = '/home/debdeep/gdrive/work/peridynamics/granular/data/3d-collision-elastic-CurrPos.png'
plt.plot(tt, ua, label = 'A')
plt.plot(tt, ub, label = 'B')
plt.xlabel(r'Time ($\mu$s)')
plt.ylabel(r'$z$-position of center (m)')
...
save_filename = '/home/debdeep/gdrive/work/peridynamics/granular/data/3d-collision-elastic-vel.png'
plt.plot(tt, va, label = 'A')
plt.plot(tt, vb, label = 'B')
plt.xlabel(r'Time ($\mu$s)')
plt.ylabel(r'$z$-velocity of center (m)')
