...
# delta = nh, h=meshsize
del_to_rad_fac = float(sys.argv[1])
mesh_to_del_fac =float(sys.argv[2])
cont_to_rad_fac =float(sys.argv[3])

delta = 1e-3/del_to_rad_fac
meshsize = delta/mesh_to_del_fac
contact_radius = 1e-3/cont_to_rad_fac
...
particles[0][1].shift([1e-3+contact_radius, contact_radius*1.1])
...
exp_b.plot(by_CurrPos=False, plot_scatter = True, plot_delta = 1, plot_contact_rad = 1, plot_bonds = 1, plot_bdry_nodes = 0, plot_bdry_edges= 0, plot_wall = 1, plot_wall_faces = False, wall_color='blue', wall_alpha=0.1, do_plot = False, do_save = 1, save_filename = 'output/img/setup.png', dotsize = 10, linewidth = 0.3, remove_axes = False, grid = False, show_particle_index=True)
